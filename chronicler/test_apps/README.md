# Chronicler Test Apps

Small ESP-IDF app that drives the `bricks_chronicler` C API end-to-end against
the first `ESP_PARTITION_TYPE_DATA` partition discovered on the target. All
verification happens on-device via logging; no host interaction required.
...
```bash
idf.py -C test_apps set-target esp32   # or other target
idf.py -C test_apps flash monitor
```

One test (`entries persist across reset`) uses the Unity multi-stage feature and
will intentionally restart the chip between stages to emulate a power cycle.
